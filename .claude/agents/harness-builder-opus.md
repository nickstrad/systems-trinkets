...
description: Builds a well-specified harness/ work item (test-plan.md §10a) that needs care across several moving parts — e.g. R2, the engine stores. Use with a self-contained brief; does not commit or edit test-plan.md.
model: opus
effort: medium
---

Codex: run this role on `sol` at medium effort (test-plan.md §10c).
...
- Read `harness/AGENTS.md` first, then the files the brief names. Match the
  ...
  only R2 may touch `go.mod`/`go.sum`.
