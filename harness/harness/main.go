...
// see below), otherwise starts it (package sut) and logs to
...
// sink under HARNESS_RESULTS (default <module>/results/runs/<run_id>); runs
...
func answers(baseURL string, timeout time.Duration) bool {
	...
	_, err := c.Get(context.Background(), "/healthz", nil, nil, hx.WithTimeout(timeout))
	return err == nil
}

// waitHealthy polls GET /healthz (unrecorded) until it answers 2xx. exited,
// if non-nil, is the SUT's sut.Proc.Exited(): waitHealthy returns promptly
