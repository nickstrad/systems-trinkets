...
func (c *Client) do(ctx context.Context, method, pathTemplate string, pathArgs map[string]string, body any, out any, o reqOpts) (*Resp, error) {
	...
	default:
		...
		if err != nil {
			return nil, fmt.Errorf("hx: marshal body: %w", err)
		}
	...
	if err != nil {
		return nil, fmt.Errorf("hx: build request: %w", err)
	}
	if contentType != "" {
	...
	if err != nil {
		return nil, fmt.Errorf("hx: %s %s: %w", method, path, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("hx: read body: %w", err)
	}
	...
	if out != nil && resp.OK() && len(respBody) > 0 {
		if err := json.Unmarshal(respBody, out); err != nil {
			return resp, fmt.Errorf("hx: decode response: %w", err)
		}
	}
	return resp, nil
}
...
func expandPath(template string, pathArgs map[string]string) (string, error) {
	...
	for i < len(template) {
		...
		if close < 0 {
			return "", fmt.Errorf("hx: unterminated {} in path template %q", template)
		}
		close += open
		key := template[open+1 : close]
		val, ok := pathArgs[key]
		if !ok {
			return "", fmt.Errorf("hx: missing path arg %q for template %q", key, template)
		}
