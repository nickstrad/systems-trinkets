...
Current design authority: [architecture.md](architecture.md). New-suite
procedure: [suite-authoring.md](suite-authoring.md). Future work:
[backlog.md](backlog.md).

## Current work

- 2026-09-12: Started the five user-authorized study-layout refactorings.
  Package and file moves preserve runtime behavior except the default output
  location, now `artifacts/runs/`. Existing saved runs move intact.


- 2026-09-12: Completed the five study-layout refactorings: `suitekit`,
  `httpclient`, and `process` packages; focused toolkit and counter-suite
  files; generated suite setup/contract/concurrency files; `artifacts/runs`
  default; current architecture, authoring, backlog, and archived history.
  Updated imports, templates, CLI help, instructions, reading guide, and
  knowledge. Moved 138 existing run files with identical SHA-256 checksums.
  The first test pass caught an accidental `sut.log` rename; corrected it
  before the final checks.
- 2026-09-12: Verification passed: `go -C harness test -race ./...`,
  `go -C harness build ./...`, `go -C harness vet ./...`,
  `go -C cli test ./...`, and `go -C examples/counter test ./...`.
  A five-file generated suite compiled in a temporary module; history reports
  read moved runs; a fresh CLI-managed memory contract run wrote under the
  new default. Current document file links resolve, the original plan remains
  verbatim in this archive, and `git diff --check` is clean. Existing unrelated
  working-tree changes were preserved. Reviewed directly; no commit created.

## Historical test plan (verbatim archive)

The former `harness/test-plan.md` is preserved below for its append-only log,
decision rationale, assignments, reviews, and completion evidence. Paths,
section references, status statements, and role instructions below describe
that historical plan; use the current documents above for new work.

---

# HTTP invariant harness — test plan

Status: **v6 (2026-09-12) — complete.** R0–R5 provide the counter worked
example on memory, SQLite, Valkey, and PostgreSQL. The completion audit added
batch runs and fixed signal/shutdown races. See §12 for evidence, §7 for
conditional backlog, and the last entry of §0 for the handoff state.

This file is the single source of truth for the harness design and living
work log. Keep it current: change decisions here first, and append to §0
when work starts, is reviewed, or lands.
...
| 2026-09-12 | Completion audit started by main: R0–R5 are committed; verify current sources and gates, repair stale as-built claims, and implement the now-triggered §7 all-targets item (four counter targets exist). Batch semantics: sorted targets, validate all before starting, run sequentially through failures, return the first nonzero exit code. Continuing direct work per the last user instruction recorded above. INV-06 wording approval requested; its bound counts acknowledgements before and after restart. |
| 2026-09-12 | Audit found two SIGINT gaps: initial SUT start was outside the publication lock, and concurrent `Sink.Close` returned before the first export finished. A new race-instrumented signal integration test reproduced missing Parquet results. Main now synchronizes startup/sink publication with shutdown; concurrent Close callers wait for one export and share its error. |
| 2026-09-12 | Audit reviewed by main: four correct-engine runs pass, all expected bug violations detected (lost-update ×4, write-behind ×3, drop-reset ×1); root tests, harness build/vet/race suite, generated-suite compilation and report checks pass. Signal regressions pass three repeated runs; no SUT listener remains on 8080–8083. Batch, lifecycle/export fixes, documentation and knowledge are ready to land in the audit commit. **Only outstanding requirement: INV-06 wording agreement (question pending).** |
| 2026-09-12 | Audit implementation committed `37231e8` (batch targets, signal shutdown/export regressions and fixes, docs and knowledge). All authorized implementation work is verified. **Waiting only for the pending INV-COUNTER-06 wording response; leave the goal active.** |
| 2026-09-12 | User instructed: if correctness needs INV-06, do it; otherwise omit it. Main retained INV-06 because the agreed crash/recovery guarantee requires detecting acknowledged writes lost on restart (write-behind passes ordinary tests but fails this check). The existing test and corrected bound already satisfy that need; no new implementation or approval step is required. Invariant status finalized under this delegated judgment; prior verification remains valid because only documentation changed. **Plan complete.** |

| 2026-09-12 | User requested a standalone idiomatic counter reference. Started extraction to `examples/counter/` as its own Go module: public counter HTTP/memory package for the existing in-process fallback, `cmd/counter`, private engine stores and test helpers. Harness targets and subprocess tests will use the sibling module; contracts and behavior stay unchanged. |

| 2026-09-12 | Counter extraction verified: `go test -race ./...` and `go vet ./...` pass in both `examples/counter/` and `harness/`; standalone command builds; all four configured targets pass, including persistent-engine crash/restart checks. Shared conformance helpers moved to `internal/storetest`, removing `testing` from the public runtime package. Binary dependency inspection confirms no harness, DuckDB, or testing dependency. README, current layout/API sections, and knowledge updated. Changes left uncommitted for this request. |

| 2026-09-12 | User clarified ownership: correct implementations are user-authored reference/application code; harness mechanics and deliberate faults belong under `harness/`. Started moving fault decorators, fault tests, and `--bug` composition into harness-owned support. Remove bug-only `Set` from the application Store contract; preserve correct-engine and harness checks. |

| 2026-09-12 | Ownership cleanup verified: counter and harness race suites and vet pass; all four correct targets pass. Harness-owned fault command rejected end to end for memory lost-update (INV-01), memory drop-reset (INV-04), and SQLite write-behind crash (INV-06); slow-server interruption regression passes. Normal binary rebuilt and checked for absence of `--bug` and harness imports. Knowledge, root/harness instructions, and reference README now explicitly assign correct implementation logic to the user and faults/toolkit machinery to harness maintainers. |
...
Ownership amendment (2026-09-12): the user builds correct implementation
logic. Harness-owned faults and fault servers live under `harness/`, never
in user/reference applications. The correct counter command has no `--bug`
flag and its Store interface has no bug-only `Set` method. Harness validation
uses `internal/counterfault` and `cmd/counter-fault`; the agreed HTTP contract
remains the user-facing integration requirement.

Current layout amendment (2026-09-12): the counter reference lives at
`examples/counter/`, module `systems-trinkets/examples/counter`, outside the
harness. The harness uses a local module replacement for its in-process
HTTP fallback and launches the standalone command for configured targets.
Earlier work-log paths below record their historical locations.
...
| Q6 | Where does it live? | `harness/` at the repo root as its **own Go module** (`systems-trinkets/harness`), so the `trinkets` notes CLI does not inherit test/parquet dependencies. `harness/results/runs/` is gitignored; `results/` also contains tracked source. Also add the built `systems-trinkets` binary to the root `.gitignore`. | **agreed** |
...
**Terminology:** "SUT" = *server under test* — the little HTTP server you write for a pattern (in any language, on any engine). External targets are tested only over HTTP. The counter suite also links its memory reference for the in-process fallback.
...
harness/
  ...
  AGENTS.md               working agreement for agents (derived from §8)
  ...
  load/                   concurrency: Closed / ClosedFor (N workers),
                          barrier start and Phase labels; Open is backlog
...
```go
// In a suite package with the usual imports; see suites/counter for full tests.
func TestMain(m *testing.M) {
    os.Exit(harness.Main(m, harness.InProcess(func() http.Handler {
        return counter.NewHandler(counter.NewMemory())
    })))
}

func TestIncrementConcurrent(t *testing.T) {
    h := harness.New(t)
    args := map[string]string{"name": "hot"}
    r := load.Closed(h, 32, 200, func(w *load.Worker) error {
        _, err := h.Client.Post(w.Ctx, "/counters/{name}/incr", args, nil, nil)
        return err
    })
    if !r.Conclusive(h) { return }
    var got struct { Value int64 `json:"value"` }
    h.Must(h.Get("/counters/{name}", args, &got))
    check.Invariant(h, "INV-COUNTER-01", got.Value == int64(r.OK2xx),
        "final value equals number of successful increments",
        map[string]any{"got": got.Value, "want": r.OK2xx})
}
```
...
|------|---------|-------|------|
| Closed-loop concurrency (N workers) | stdlib goroutines + `sync.WaitGroup` in `load.Closed`; a channel barrier so all workers fire at once | | `rakyll/hey` (CLI only) |
...
Takeaway: day one needs only the stdlib, TOML, and a Parquet path (§6). Everything else is additive and slots into an existing package.

The reference service is a sibling module, outside this tree:

```text
examples/counter/
  go.mod, go.sum
  cmd/counter/main.go
  store.go, handler.go, memory.go
  store/open.go
  counter_test.go
  internal/store/{sqlite,valkey,postgres}/
  internal/storetest/
```
...
test goroutines ──rows──▶ chan ──▶ collector goroutine ──▶ Appender (in-memory DuckDB, 5 tables)
                                                                 │
                       TestMain exit / SIGINT / SIGTERM ───────────▶ COPY each table TO results/runs/<run_id>/<table>.parquet
                                                                 │
...
| built 2026-09-12 (completion audit) | `harness run <pattern> --all-targets` | Validate every matching `targets/<pattern>-*.toml` before starting; run in filename order, one result directory each, continue through failures and return the first nonzero exit code. Also accept `run --all-targets <pattern>`. |
...
## 8. Working agreement (source for `AGENTS.md`)
...
5. Implement the reference (or the user's implementation) and run: `harness run <pattern> --target targets/<file>.toml`. Keep fault variants under harness-owned support and prove the suite catches them; do not add fault logic to the user implementation.
...
## 9. Package APIs (as built 2026-09-12)
...
func (s *Sink) Close(ctx context.Context) error   // drain, write run row, COPY every table to <dir>/<table>.parquet; concurrent callers wait for the same export/error
...
// examples/counter (separate module) — the reference counter as a library. Correct application logic only.
package counter
...
func storetest.Run[S storetest.Store](t *testing.T, open func() S)   // ★ conformance test every engine runs (R2 callers): sequential semantics, Incr atomic under 32×200 callers, isolation; wipes and closes the store
// engines, private packages composed by the public store.Open factory:
//   internal/store/sqlite.Open(dsn)   modernc.org/sqlite (same driver as the notes CLI; pure Go); WAL, busy_timeout; INSERT … ON CONFLICT DO UPDATE SET value = value + excluded.value RETURNING value
//   internal/store/valkey.Open(dsn)   github.com/valkey-io/valkey-go; INCRBY / GET / DEL / FLUSHDB on the DB index in the DSN
//   internal/store/postgres.Open(dsn) github.com/jackc/pgx/v5 via database/sql; same upsert … RETURNING; TRUNCATE
```

`cmd/counter` (the reference binary, `examples/counter/cmd/counter`):

```
counter --addr 127.0.0.1:8080 --engine memory|sqlite|valkey|postgres [--dsn …]
```
...
`NewHandler(engine)`. `store.Open(engine, dsn, addr)` selects the correct engine.

Harness-only fault support is in `internal/counterfault`: `Wrap(store, bug)`
returns the selected decorator. Its overwrite adapter implements the bug-only
Set operation using Del/Incr, so user stores need no fault-specific methods.
`cmd/counter-fault` composes that wrapper with the correct store factory and
HTTP handler. It accepts `--bug none|lost-update|drop-reset|write-behind|slow`;
write-behind flushes every 1 s and slow sleeps 20 ms. Its unit tests and the
slow-server interruption regression live in the harness module.

`cmd/harness`:

```
harness run <pattern> (--target targets/x.toml | --url http://… | --all-targets)  [-- go test flags]
...
hint. A single `harness run` returns `go test`'s exit code. `--all-targets` validates all matching files first, runs them in filename order with a separate run ID and report each, continues through failures, and returns the first nonzero code. Explicit CLI target selection overrides inherited target metadata.
...
| R3 ✅ | `sut` package (§9) + `harness.Main` integration: start `Target.Cmd` (cwd relative to the target file, env merged) before the health wait, log to `<results dir>/sut.log`, stop at exit / SIGINT / SIGTERM; `H.Restartable`, `H.Restart`; `cmd/harness run` prints "started SUT pid …" and fails fast if the process exits before health. Unit tests spawn `example-sut/cmd/counter --engine memory` (build once in `TestMain`; sqlite lands in parallel with R2, and state-across-restart is an engine property covered by R2's `StoreTest` and R4): start → healthy; kill → connection refused; restart → healthy with a new pid; Stop with grace → clean exit. Files: `sut/*`, `harness/main.go`, `harness/h.go`, `cmd/harness/run.go` only. | first pass: builder-sonnet (sonnet high / luna high); review fixes: builder-opus (opus medium / sol medium — reassigned by the user 2026-09-12, see §0) | reviewer (opus high / sol high — process-group kill, reaping, and the SIGINT paths are where races hide) | tests green with `-race`; `harness run counter --target targets/counter-go-sqlite.toml` starts and stops the SUT itself |
| R4 ✅ | INV-COUNTER-06 in `INVARIANTS.md` (agree wording with the user first): *After the SUT is killed and restarted mid-load, the final value is bounded by `all-2xx ≤ final ≤ all-2xx + errored`: every acknowledged increment survives, and no increment is acknowledged before it is committed.* Guaranteed by: respond only after the store's atomic write returns (application discipline); the store's own persistence. Kind: crash, must. Test: `Closed` workers on one name in a load goroutine; `h.Restart()` once from the test goroutine after ~⅓ of the expected requests (count-triggered, not time-triggered); tally all 2xx before and after restart and transport errors around the kill; `GET` after restart; skip when `!h.Restartable()`. Counter's crash test lands here rather than on a pattern that does not exist yet. | main (this session / astra — retained under user-authorized correctness judgment) | reviewer (opus high / sol high) | passes on `--engine sqlite\|valkey\|postgres --bug none`; fails INV-06 on `--bug write-behind` (each engine); skips on memory / `--url` targets |
...
This is the original breakdown; its panic-safe export claim was withdrawn
in R3. Current guarantees are in §6 and the audit in §12.
...
## 12. Completion audit (2026-09-12)

Scope: the agreed toolkit and four-engine counter worked example (A–H,
R0–R5), plus the now-triggered all-targets backlog item. Conditional items
remain conditional: rate-limiter open-loop load, optional performance gates,
on-disk results, and a linearizability checker are not counter milestones.
Q7 remains explicitly deferred; no new pattern was requested.

| Requirement | Current evidence | Result |
|------------|------------------|--------|
| Separate module, tracked source, direct dependencies (R0/A) | `harness/go.mod`, `.gitignore`, current tracked package files; `go build ./...`, `go vet ./...`, and uncached `go test -race -count=1 ./...` in the harness pass. Root `go test ./...` also passes. | verified |
| Results rows, concurrency-safe collector, five Parquet files, SQL p99, 100k samples under 1 s (A/§6) | `TestSinkExportsParquet`; report tests for summary, latency, checks, compare, history; concurrent-close regression proves every returning caller sees exported files. | verified |
| HTTP sampling, reset/health, load barrier/cancellation, named checks (B–D/F) | `hx`, `load`, `harness` unit tests and actual counter suite runs. Counter's `TestMain` uses `InProcess`; generated suites skip without a target until they have a fallback. | verified |
| Reference library, handler contract, memory and decorators (E/R1) | Store/handler/decorator tests, in-process suite, all pass under race detector. `lost-update` fails INV-01/02/05 on all four engines; `drop-reset` fails INV-04 on memory in current audit runs. | verified |
| Four stores, health pings, conformance, targets, compose, reports (H/R2) | Both compose services healthy; store tests and `run counter --all-targets` pass. History shows passing runs `20260913T002359Z-kebb` (memory), `20260913T002401Z-6hih` (postgres), `20260913T002406Z-n6kc` (sqlite), `20260913T002408Z-9h8k` (valkey). | verified |
| SUT start/kill/restart/stop, cwd/env, stale listener rejection, early exits, SIGINT (R3) | `sut` tests and `harness` subprocess tests; real SQLite crash run; race-instrumented interruption regression repeated three times. Startup/publication and shutdown now share a mutex, and all Close callers wait for export. | verified |
| Crash invariant mechanics and bug detection (R4) | Real crash tests pass on SQLite/Valkey/Postgres; write-behind fails INV-06 on each. Memory skips. `TestCrashSkipsExternalSUT` exercises the URL-only skip. Bounds include acknowledgements both before and after restart. | verified |
| CLI run/report/sql/targets/new-suite (G) | CLI tests, actual batch/history/targets commands, generated `audit-smoke` suite compiled and passed in a temporary module. Separate run IDs/results and forwarded Go flags after an earlier target failure are covered by a subprocess regression. | verified |
| Current docs and knowledge (R5) | Corrected in-process behavior, actual APIs/skeleton, implemented engines, batch semantics, crash bound and signal/export limitations; knowledge records the reproduced shutdown/export races. Historical log entries remain append-only. | verified |
| All-targets trigger (§7: at least three targets) | Four counter targets; sorted discovery, prevalidation, sequential execution, distinct runs/reports, continue-on-failure and first nonzero exit code implemented and tested. | verified |
| Contract/invariant agreement (R4/§8) | INV-01–05 are agreed. User authorized retaining INV-06 if needed for correctness; retained because ordinary concurrency tests cannot detect acknowledged writes lost on SUT restart. | resolved |

Review was performed directly by main, continuing the last user instruction
recorded in §0 to stop agents and finish directly. No independent agent
review is claimed. The user delegated the remaining correctness decision: INV-06 is retained,
and the agreed implementation scope is complete. Conditional backlog in §7
remains available for future patterns or explicit requests.


## Study-layout refactoring (2026-09-12)

User authorized all five proposed changes: rename toolkit packages to suitekit, httpclient, and process; split lifecycle and counter-suite files by responsibility; move generated runs to artifacts/runs; separate current architecture, suite authoring, backlog, and historical work log under docs/. Current design authority becomes docs/architecture.md. Existing runs and historical records are preserved. Work started.
