...
func TestMain(m *testing.M) {
	...
	build := exec.Command("go", "build", "-o", counterBin, "./cmd/counter")
	build.Dir = filepath.Join(moduleRoot(), "..", "examples", "counter")
	if out, err := build.CombinedOutput(); err != nil {
		fmt.Printf("sut: build standalone counter: %v\n%s\n", err, out)
		os.Exit(1)
	}
...
// local (rather than importing package suitekit, which imports process) to avoid
...
func TestKillReachesGrandchild(t *testing.T) {
	...
	spec := Spec{
		Cmd:    []string{"go", "run", "./cmd/counter", "--addr", addr, "--engine", "memory"},
		Dir:    filepath.Join(moduleRoot(), "..", "examples", "counter"),
		Env:    os.Environ(),
		Stdout: &log,
		Stderr: &log,
	}
...
// suitekit.Main relies on: its signal handler may Stop the process while a
