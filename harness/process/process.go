// Package process starts, stops and restarts one system-under-test process.
// It is independent of package suitekit (suitekit imports process, not the
// reverse), so it takes a Spec rather than a suitekit.Target — the caller
// (suitekit.Main) is responsible for turning a Target into a Spec (resolving
...
// exec.Cmd's default (discarded); suitekit.Main points them at
// <results dir>/sut.log.
type Spec struct {
	Cmd    []string
	Dir    string
...
// only signals the group and waits on exited. suitekit.Main needs that —
